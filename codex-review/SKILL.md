...
Two models, one document, a bounded argument. **Claude is the author and arbiter. Codex is a read-only critic** that can read the repo and the doc but cannot touch a single file. The human enters at exactly two points: kickoff and final sign-off.
...
- **Echo the active model and reasoning effort before Round 1** so the user can confirm. Codex resolves config with precedence CLI `-c` > project `.codex/config.toml` (repo root, trusted projects) > `~/.codex/config.toml` — so check in that order: any `-c` override this skill passes, then the project file if present, then the user file; report the first `model` / `model_reasoning_effort` found and WHICH layer it came from (e.g. `model gpt-5.6-sol (user config), effort xhigh (project config)`). If the user objects, stop before burning a round.
...
| `BASE_REF` | inferred (`impl` only) | Git ref the implementation diff is measured from: the commit the implementation started at (merge-base with main, or last commit before the working-tree changes). Echo it — a wrong base reviews the wrong diff. |
| `FEATURE` | inferred | Feature slug keying the session. Infer from the filename: strip date prefix, extension, and trailing `-spec`/`-plan`/`-lot-x` qualifiers so spec and plan of the same feature share a slug (`2026-08-30-provenance-receipt-lot-a.md` → `provenance-receipt`). Echo the inferred slug — the user can override with `feature=<slug>`. |
| `reasoning` | `inherit` | Reasoning effort for the reviewer: `low`/`medium`/`high`/`xhigh`/`max` (upper tiers model-dependent). `inherit` = don't pass anything, Codex uses its config default. Any other value: append `-c model_reasoning_effort="<value>"` to BOTH the fresh and resume commands, and echo the effective value at kickoff. |
| `MAX_ROUNDS` | `5` | Hard cap on review rounds THIS invocation. The loop ALWAYS terminates at this. |
| `STATE_FILE` | `.codex-review/sessions.json` | Per-feature session registry at the repo root. |
...
2. **State file.** The most recently `updated` entry in `STATE_FILE` whose `last_verdict` is not `APPROVED` — an argument in progress is the natural default to continue.
...
5. **Resume fails** (dead/expired thread, garbage id) → treat as fresh: new `codex exec`, overwrite the entry with the new `thread_id`, note the reset in `LOG_FILE`. Never retry a failed resume blind.
...
Write the prompt to a temp file — never inline-quote it.

**Impl-mode round mechanics differ in one place:** on `VERDICT: REVISE`, Claude arbitrates the findings as usual, but fixes the CODE (not the doc) — apply accepted fixes, rerun the affected tests, then resume with *"Findings addressed (or rejected with reasons below). Re-review the diff since `<BASE_REF>`."* Cap `MAX_ROUNDS` at **2** by default in impl mode (initial review + one re-inspection — mirrors claudex-loop's `MAX_INSPECTION_ROUNDS`; delegating trivia back and forth burns more than it saves). At the cap, Claude finishes the remaining accepted fixes directly and logs it.
...
**If FRESH** (creates the session — capture `thread_id`):
...
Parse `thread_id` from the `{"type":"thread.started","thread_id":"..."}` line (first line of `STREAM_FILE`) → write it to `STATE_FILE` immediately. The critique text lands in `/tmp/codex-verdict.txt`.

**If RESUMED / INHERITED** (entry exists — echo the id visibly into the command before running):
...
- `{"type":"thread.started","thread_id":...}` — session up (fresh calls only).
...
All subsequent rounds resume the SAME `THREAD_ID` (resume command above, "I revised the doc" prompt). Each round, after Codex returns:
...
- It NEVER rejects a `[defect]` whose failure scenario is concrete. Simplicity governs the SHAPE of the fix, not whether the defect gets fixed.
...
- **Good-enough exit.** If a round's findings contain zero `[defect]`s and only immaterial simplifications, Claude may declare convergence and go to Resolution even without `VERDICT: APPROVED` — presenting the residual list to the user. Iterating to zero nitpicks is the infinite loop this rule exists to kill.
...
- Codex is read-only EVERY round — `-s read-only` on fresh calls, `-c sandbox_mode="read-only"` on every resume. It never writes.
...
- Code only after human gate #2, and only from an APPROVED plan (an approved spec is not an implementation license).
