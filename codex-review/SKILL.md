...
>
> **Timeout guard — inactivity, not wall clock.** Real rounds routinely exceed 10 minutes (round-1 durations observed on 400-800-line docs: 13.8, 18.4 and 10.4 min; a 10-min absolute ceiling killed one of them mid-exploration and the verdict had to be fetched by a second resume). So: ALWAYS launch rounds in the background (the Bash tool's foreground timeout caps at 10 min) and enforce two bounds from the polling loop — a **stall bound**: no new event appended to `STREAM_FILE` for 10 min while the process is alive → kill it, failed run; and a **hard cap** of 45 min per round as a runaway guard. Never wrap the command in `timeout`/`gtimeout` with a short value. A trip = failed run: stop and tell the user (stderr tail + last stream events), don't retry blind.

### Step 3 — The loop (rounds 2..MAX of this invocation)

All subsequent rounds resume the SAME `THREAD_ID` with the resume prompt matching `DOC_TYPE`: doc revision ("The doc has been revised…"), stage inheritance, or impl re-review ("Re-review the diff since `<BASE_REF>`") — see Steps 1-2; never send the doc-revision prompt on an impl round.

**Did the revision actually land? Check before every resubmission.** Spec/plan rounds: compute `shasum -a 256 "$DOC_FILE"` and compare it with the hash of the doc as submitted in the previous round (held in memory within an invocation; `doc_sha256` in `STATE_FILE` across invocations, written with each round's verdict). Identical hash → the edits did not reach the file (seen twice in real use: a background edit script failed on an assert, the unchanged doc was resubmitted, and a round was burnt for Codex to answer "the file is still rev. N"). Stop, fix the doc, verify the hash moved, then resume. Impl rounds: confirm the accepted fixes are in the working tree (`git diff <BASE_REF> --stat` changed since the previous round) before resuming. Each round, after Codex returns:
...
2. Grep the last line for the verdict token.
   ...
   - `VERDICT: REVISE` → Claude decides **what's actually worth acting on** (Claude is final arbiter — Codex advises, it does not command). Revise `DOC_FILE`. Append to `LOG_FILE`: `### Claude's response` + what changed, what was rejected, why. Update `STATE_FILE` (verdict + `doc_sha256` of the doc as just reviewed). Increment round.
...
- Weigh a finding by projecting it onto a plausible production situation, never by today's affected population. "Zero tenants / rows / users hit this right now" is not a reason to reject or reduce during development — the review protects the future production state, and the population will change after the feature ships. The converse holds too: don't invent production scenarios the spec doesn't target to justify adding mechanism. And check the nominal path first: the configuration most deployments will actually run deserves at least the scrutiny given to edge cases (a real escape: 14 rounds on edge cases, then the mono-source case — 100 % of production — turned a 10-min quarantine into a global 503).
